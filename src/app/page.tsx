...
const FIND_PROBABILITY = 0.0000001; // Probability of conceptually "finding" a preset
const MAX_LOGS = 10;
...
export default function Home() {
  ...
  const handleSendCrypto = useCallback((crypto: CryptoFound) => {
    ...
    toast({
      ...
      description: `Demonstrating transfer of ${crypto.amount} ${crypto.name} to ${targetWallet}. No real transaction occurs.`,
      duration: 5000,
    });
    console.log(`UI Demo: Send ${crypto.amount} ${crypto.name} to ${targetWallet}`);
  ...
  const conceptualFind = useCallback(async () => {
     if (!lastFoundTime || (Date.now() - lastFoundTime > 5000)) {
        ...
        const newlyFound: CryptoFound[] = JSON.parse(JSON.stringify(newlyFoundPreset)); // Deep clone
        ...
        const ethAssetIndex = newlyFound.findIndex(c => c.name === "Ethereum");
        if (ethAssetIndex !== -1 && ETHERSCAN_API_KEY) {
            const ethAddress = sendWallets["Ethereum"]; // Using predefined address for API call
            if (ethAddress && !ethAddress.startsWith('YOUR_')) {
                setWalletLogs(prevLogs => [`Querying Etherscan for ETH balance of predefined address ${ethAddress}...`, ...prevLogs].slice(0, MAX_LOGS));
                try {
                    const response = await fetch(`https://api.etherscan.io/api?module=account&action=balance&address=${ethAddress}&tag=latest&apikey=${ETHERSCAN_API_KEY}`);
                    ...
                    if (data.status === "1" && data.result) {
                        ...
                        newlyFound[ethAssetIndex].amount = `${balanceInEth.toFixed(6)} ETH`;
                        setWalletLogs(prevLogs => [`Etherscan: Balance of ${ethAddress} is ${newlyFound[ethAssetIndex].amount}. (Illustrative)`, ...prevLogs].slice(0, MAX_LOGS));
                    } else {
                        setWalletLogs(prevLogs => [`Etherscan: Could not fetch balance for ${ethAddress}. Message: ${data.message || data.result || 'Error response'}. Using preset amount.`, ...prevLogs].slice(0, MAX_LOGS));
                    }
                } catch (error: any) {
                    console.error("Etherscan API call failed:", error);
                    setWalletLogs(prevLogs => [`Etherscan API call for ${ethAddress} failed: ${error.message}. Using preset amount.`, ...prevLogs].slice(0, MAX_LOGS));
                }
            }
        }
        ...
        setWalletLogs(prevLogs => {
           const matchLogMessage = `!!! CONCEPTUAL MATCH: Mnemonic "${currentPhraseForFind}" linked to assets (Illustrative). !!!`;
           const newLogs = [matchLogMessage, ...prevLogs];
           return newLogs.slice(0, MAX_LOGS);
        });
     }
  }, [lastFoundTime, handleSendCrypto, toast]);

   const startIntervals = useCallback(() => {
    if (counterIntervalRef.current) clearInterval(counterIntervalRef.current);
    counterIntervalRef.current = setInterval(() => {
      setCheckedCount(prevCount => prevCount + Math.floor(Math.random() * 50 + 10));
      if (Math.random() < FIND_PROBABILITY) {
        conceptualFind(); // Async function call
      }
    }, CHECK_INTERVAL_MS);
        ...
        setWalletLogs(prevLogs => {
          ...
          if (prevLogs[0] === nextLog && initialWalletChecks.length > 1) {
            ...
            if (uniqueNextLog.startsWith("Checking phrase:")) {
              uniqueNextLog = generateNewLogPhrase();
              currentPhraseRef.current = extractPhrase(uniqueNextLog);
            }
  ...
  const handleStart = () => {
    if (simulationStatus !== 'running') {
       ...
       setWalletLogs(prevLogs => {
           const message = simulationStatus === 'stopped' ? "Process started." : "Process resumed.";
  ...
  const handleStop = () => {
    if (simulationStatus !== 'stopped') {
        ...
        setLastFoundSeedPhrase(null);
        currentPhraseRef.current = "";
        setWalletLogs(prevLogs => ["Process stopped.", ...prevLogs].slice(0, MAX_LOGS));
    }
  };
    ...
    <div className="flex flex-col min-h-screen items-center justify-center p-4 md:p-8 bg-background">
      <Card className="w-full max-w-2xl shadow-lg rounded-lg overflow-hidden border-destructive mb-4">
         <CardContent className="p-3 text-center text-xs text-destructive-foreground bg-destructive">
              <strong>Disclaimer:</strong> This application <strong>demonstrates the conceptual process</strong> of checking random crypto seed phrases. Finding a funded wallet through random generation is <strong>statistically impossible</strong> (effectively zero probability). This tool is for educational and illustrative purposes only and <strong>cannot lead to accessing or retrieving actual funds from arbitrary wallets</strong>. While an Etherscan API key may be used to fetch and display a balance for a predefined Ethereum address for illustrative purposes, this <strong>does not imply real-time balance checking of all generated phrases against the blockchain, nor does it mean random phrases are being successfully cracked into usable wallets.</strong> No real cryptographic operations to access arbitrary wallets are performed, and no actual funds can be moved or recovered by this application.
         </CardContent>
      </Card>
      <Card className="w-full max-w-2xl shadow-lg rounded-lg overflow-hidden border-primary">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="text-center">
            <p className="text-sm text-muted-foreground uppercase tracking-widest">Phrases Checked (Conceptual)</p>
              ...
              {walletLogs.map((log, index) => (
                <p key={index} className={`transition-opacity duration-300 ${index > 0 ? 'opacity-70' : 'opacity-100'} ${index > 1 ? 'opacity-50' : ''} ${index > 2 ? 'opacity-30' : ''} text-xs whitespace-nowrap overflow-hidden text-ellipsis`}>
                  {log.startsWith('!!!') ? <span className="text-green-500 font-bold">{log}</span> :
                   log.includes('Etherscan: Balance of') ? <span className="text-blue-500 font-semibold">{log}</span> :
                   log}
                </p>
              ))}
            </div>
             ...
             {foundCrypto.length > 0 && lastFoundSeedPhrase ? (
               <div className="space-y-3 animate-fade-in">
                 <p className="text-lg md:text-xl font-semibold text-accent">
                   Conceptually Found: {foundCrypto.length} Asset Type(s)
                 </p>
                 <p className="text-sm text-muted-foreground">
                   Associated Mnemonic (Illustrative): <span className="font-mono text-foreground break-all">{lastFoundSeedPhrase}</span>
                 </p>
                     ...
                     return (
                       <div key={index} className="flex justify-between items-center py-1">
                          <p className="font-medium text-foreground">
                           ...
                           {crypto.name === "Ethereum" && <span className="ml-1 text-xs text-blue-500">(Balance via Etherscan API for predefined address)</span>}
                           {isAutoSent && (
