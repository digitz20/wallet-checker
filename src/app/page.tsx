...
const FIND_PROBABILITY = 0.0000001; 
const MAX_LOGS = 8; 
...
const generateSimulatedPhraseLog = (): string => {
  ...
  return phrase + ' (simulation)';
};

// Helper function to extract the phrase from a log entry
const extractPhrase = (logEntry: string): string => {
  const prefix = "Checking phrase: ";
  const suffix = " (simulation)";
  if (logEntry.startsWith(prefix) && logEntry.endsWith(suffix)) {
    return logEntry.substring(prefix.length, logEntry.length - suffix.length).trim();
  }
  return ""; 
};
...
export default function Home() {
  ...
  const handleSendCrypto = useCallback((crypto: CryptoFound) => {
    ...
    toast({
      ...
      description: `Simulating transfer of ${crypto.amount} ${crypto.name} to ${targetWallet}`, 
      duration: 5000, 
    });
     ...
     if (!lastFoundTime || (Date.now() - lastFoundTime > 5000)) { // Ensure some delay between finds
        ...
        newlyFound.forEach(crypto => {
          if (AUTO_SEND_ASSETS.includes(crypto.name)) {
            ...
            if (targetWallet && !targetWallet.startsWith('YOUR_')) {
              setTimeout(() => handleSendCrypto(crypto), 500); 
            } else {
      ...
      <Card className="w-full max-w-2xl shadow-lg rounded-lg overflow-hidden border-primary">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="text-center">
            <p className="text-sm text-muted-foreground uppercase tracking-widest">Checked (Simulated)</p>
             ...
             {foundCrypto.length > 0 && lastFoundSeedPhrase ? (
               <div className="space-y-3 animate-fade-in">
                ...
                <div className="text-center text-muted-foreground italic py-4">
                  {simulationStatus === 'stopped' ? 'Simulation stopped. Wallet checks reset.' :
